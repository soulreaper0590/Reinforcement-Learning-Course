            ...
            for i, row in enumerate(f):
                ...
                for k in range(10):
                    for j, col in enumerate(row):
                        for r in range(100):
                            if col == "4":
                                self.start.append([10*i + k, 100*j + r])  # self.n * i + j
                            elif col == "3":
                                self.goals.append([10*i + k, 100*j + r])
                            elif col == "-1":
                                self.puddle1.append([10*i + k, 100*j + r])
                            elif col == "-2":
                                self.puddle2.append([10*i + k, 100*j + r])
                            elif col == "-3":
                                self.puddle3.append([10*i + k, 100*j + r])
                            elif col == "1":
                                self.walls.append([10*i + k, 100*j + r])
            self.m = 10*i +k+ 1
        if len(self.goals) == 0:
    ...
    def _step(self, action):
        ...
        if self.state in self.goals:  # Checking if reaached goal or not
            ...
            if np.random.binomial(1, 0.9, 1)[0] == 1:
                ...
                #action = temp_action[action_taken]
