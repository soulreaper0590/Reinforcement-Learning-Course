        ...
        if 0:
            ...
            with tf.name_scope('output'):
                Wo = tf.Variable(tf.random_uniform(
                    [self.HIDDEN2_SIZE, self.output_size], -1.0, 1.0), name='Wo')
                bo = tf.Variable(tf.random_uniform(
                    [self.output_size], -1.0, 1.0), name='bo')
                # , name="Q_values")
                self.Q = tf.squeeze(tf.matmul(h2, Wo) + bo)

            self.weights = [W1, b1, W2, b2,  Wo, bo]
